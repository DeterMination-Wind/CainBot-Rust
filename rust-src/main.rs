...
async fn run() -> Result<()> {
    ...
    let config_path = match std::env::var("CAINBOT_CONFIG") {
    ...
    let qa_client = OpenAiChatClient::new(
    ...
    let _translator = OpenAiTranslator::new(
    ...
    let event_runtime_store = runtime_config_store.clone();
    let enabled_static_groups = config.qa.enabled_group_ids.clone();
    napcat_client
...
// 先把“群邀请相关状态判断”搬过来，后续再把自动审批和轮询补齐。
